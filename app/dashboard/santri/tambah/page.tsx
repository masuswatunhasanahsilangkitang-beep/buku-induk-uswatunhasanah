...
export default function EmisBukuIndukPage() {
  ...
  const pendidikanOptions = [
    "Tidak Sekolah",
    "SD/MI/Ula/Sederajat",
    "SMP/MTs/Wustha/Sederajat",
    "SMA/MA/Ulya/Sederajat",
    "D1",
    "D2",
    "D3",
    "D4/S1",
  ...
  // ================= STATE TAB 1: DATA SISWA (DIBERSIHKAN) =================
  ...
  // ================= STATE TAB 2: DATA ORANG TUA (DIBERSIHKAN) =================
  ...
  const [berkasOrangTua, setBerkasOrangTua] = useState({
    ...
    fileKks: null as string | null,
    filePkh: null as string | null,
  });

  // ================= STATE TAB 3: DATA ALAMAT (DIBERSIHKAN) =================
  const [alamatAyah, setAlamatAyah] = useState({
    luarNegeri: false, kepemilikanRumah: "Milik Sendiri", provinsi: "", kabupaten: "", kecamatan: "", kelurahan: "", rt: "", rw: "", alamatLengkap: "", kodePos: "",
  });
  ...
  const [alamatSiswa, setAlamatSiswa] = useState({
    statusTempatTinggal: "Bersama Orang Tua", provinsi: "", kabupaten: "", kecamatan: "", kelurahan: "", rt: "", rw: "", alamatLengkap: "", koordinat: "", kodePos: "", jarak: "Kurang dari 5 Km", transportasi: "Jalan Kaki", waktuTempuh: "10-19 menit",
  });

  // ================= STATE TAB 4: AKTIVITAS BELAJAR (DIBERSIHKAN) =================
  ...
  useEffect(() => {
    if (waliOption === "Sama dengan ayah kandung") {
      ...
      setWali({ ...ibu });
    } else if (waliOption === "Lainnya") {
      setWali({
        namaLengkap: "", kewarganegaraan: "WNI", nik: "", tempatLahir: "",
        tanggalLahir: "", pendidikan: "D4/S1", pekerjaan: "Tidak Bekerja", nomorHp: "",
      });
    }
  }, [waliOption, ayah, ibu]);
  ...
  // ================= HANDLERS (VALIDATION & API INTEGRATION) =================
  const validateForm = () => {
  ...
  const handleAddAktivitas = (e: React.FormEvent) => {
    e.preventDefault();
    if (!aktivitasForm.ta || !aktivitasForm.nsm) {
      alert("Mohon lengkapi data Tahun Ajaran dan NSM"); return;
    }
    if (editAktivitasIndex !== null) {
      ...
      setAktivitasList([...aktivitasList, aktivitasForm]);
    }
    setShowAktivitasForm(false);
    setEditAktivitasIndex(null);
    setAktivitasForm({ ta: "", tanggal: "", nsm: "", jenjang: "", tingkat: "", jurusan: "", rombel: "", status: "Aktif", ket: "" });
  };

  const openEditAktivitas = (index: number) => {
    setAktivitasForm(aktivitasList[index]);
    setEditAktivitasIndex(index);
    setShowAktivitasForm(true);
  };

  const deleteAktivitas = (index: number) => {
    if(window.confirm("Hapus data Aktivitas Belajar ini?")) {
      setAktivitasList(aktivitasList.filter((_, i) => i !== index));
    }
  };
  ...
  const handleAddBeasiswa = (e: React.FormEvent) => {
    e.preventDefault();
    if (!beasiswaForm.tahun || !beasiswaForm.namaBantuan) {
      alert("Mohon lengkapi data beasiswa (Tahun dan Nama Bantuan wajib diisi)"); return;
    }
    if (editBeasiswaIndex !== null) {
      ...
      setBeasiswaList([...beasiswaList, beasiswaForm]);
    }
    setShowBeasiswaForm(false);
    setEditBeasiswaIndex(null);
    setBeasiswaForm({ tahun: "", kategori: "", namaBantuan: "", namaInstansi: "", jenisInstansi: "", jangkaWaktu: "", nominal: "" });
  };

  const openEditBeasiswa = (index: number) => {
    setBeasiswaForm(beasiswaList[index]);
    setEditBeasiswaIndex(index);
    setShowBeasiswaForm(true);
  };

  const deleteBeasiswa = (index: number) => {
    if(window.confirm("Hapus data Beasiswa ini?")) {
      setBeasiswaList(beasiswaList.filter((_, i) => i !== index));
    }
  };
  ...
  const handleAddPrestasi = (e: React.FormEvent) => {
    e.preventDefault();
    if (!prestasiForm.tahun || !prestasiForm.namaLomba) {
      alert("Mohon lengkapi data prestasi (Tahun dan Nama Lomba wajib diisi)"); return;
    }
    if (editPrestasiIndex !== null) {
      ...
      setPrestasiList([...prestasiList, prestasiForm]);
    }
    setShowPrestasiForm(false);
    setEditPrestasiIndex(null);
    setPrestasiForm({ tahun: "", namaLomba: "", bidangLomba: "", namaPenyelenggara: "", lombaTingkat: "", peringkat: "" });
  };

  const openEditPrestasi = (index: number) => {
    setPrestasiForm(prestasiList[index]);
    setEditPrestasiIndex(index);
    setShowPrestasiForm(true);
  };

  const deletePrestasi = (index: number) => {
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div>
          <nav className="text-xs text-gray-400 space-x-1 mb-1">
            ...
            <span className="font-semibold text-gray-600">Tambah Siswa</span>
          </nav>
      ...
      {activeTab === "DATA SISWA" && (
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-200">
            <button type="button" className="px-6 py-2.5 rounded-lg border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors">Batal</button>
            <button 
              type="submit" 
              disabled={isSubmitting}
              className={`px-6 py-2.5 rounded-lg text-white text-sm font-bold shadow-sm transition-colors flex items-center gap-2 ${isSubmitting ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
              {isSubmitting ? (
                <>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm space-y-8">
          ...
          <div className="flex justify-between items-center pt-6 border-t border-gray-200">
            ...
            <button 
              type="submit" 
              disabled={isSubmitting}
              className={`px-8 py-2.5 rounded-lg text-white text-xs font-bold shadow-sm transition-all uppercase tracking-wider flex items-center gap-2 ${isSubmitting ? 'bg-blue-400 cursor-wait' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
      ...
      {activeTab === "DATA ALAMAT" && (
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl border border-gray-200 shadow-sm space-y-8">
          ...
          <div className="flex justify-between items-center pt-6 border-t border-gray-200">
            ...
            <button type="submit" className="px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg shadow-sm transition-all uppercase tracking-wider">
              SIMPAN DATA ALAMAT
            </button>
          </div>
        </form>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-6">
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tidakAda}
                    onChange={() => handleDisabilitasChange("tidakAda")}
                ...
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tunaNetra}
                    onChange={() => handleDisabilitasChange("tunaNetra")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
                ...
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tunaRungu}
                    onChange={() => handleDisabilitasChange("tunaRungu")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
                ...
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tunaDaksa}
                    onChange={() => handleDisabilitasChange("tunaDaksa")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
                ...
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tunaGrahita}
                    onChange={() => handleDisabilitasChange("tunaGrahita")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
                ...
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tunaLaras}
                    onChange={() => handleDisabilitasChange("tunaLaras")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
                ...
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.lainnya}
                    onChange={() => handleDisabilitasChange("lainnya")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
                  <span>Lainnya</span>
                </label>
                <label className="flex items-center space-x-3 text-xs font-semibold text-gray-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={disabilitas.tunaWicara}
                    onChange={() => handleDisabilitasChange("tunaWicara")}
                    className="w-4 h-4 text-blue-600 rounded bg-gray-100 border-gray-300 focus:ring-blue-500"
                  />
  ...
  function isWaliFieldDisabled() {
    return waliOption === "Sama dengan ayah kandung" || waliOption === "Sama dengan ibu kandung" || waliOption === "Tidak Memiliki Wali";
  }
}